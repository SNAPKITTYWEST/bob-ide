...
const xml_compiler_bridge_1 = require("./xml-compiler-bridge");
// Import all 5 backend components
const executor_1 = require("./executor");
const lisp_vm_1 = require("./lisp-vm");
const mcp_router_1 = require("./mcp-router");
const agent_pool_1 = require("./agent-pool");
const worm_chain_1 = require("./worm-chain");
const app = (0, fastify_1.default)({ logger: true });
const wasmEngine = new wasm_1.WasmEngine();
// Initialize WORM chain on startup
await worm_chain_1.wormChain.initialize();
// Register plugins
await app.register(websocket_1.default);
await app.register(static_1.default, {
    root: path_1.default.join(process.cwd(), 'dist'),
});
...
app.register(async (fastify) => {
    fastify.get('/api/terminal/:id/ws', { websocket: true }, (socket, req) => {
        ...
        const shell = (0, child_process_1.spawn)('bash', [], {
            cwd: session.cwd,
            env: session.env,
            pty: true,
        });
...
app.post('/api/grep', async (req) => {
    ...
    try {
        ...
        const output = (0, child_process_1.execSync)(`grep -n ${flags} "${pattern}" "${searchPath}"`, {
            cwd: process.cwd(),
            encoding: 'utf-8',
        }).trim();
        return {
...
app.post('/api/xml/compile', async (req) => {
    try {
        const result = await (0, xml_compiler_bridge_1.compileNaturalLanguageToXML)(req.body);
        return {
            ok: true,
            ...result,
        };
...
app.post('/api/xml/control-model', async (req) => {
    try {
        const result = await (0, xml_compiler_bridge_1.controlModelViaXML)(req.body);
        return {
            ok: true,
            ...result,
        };
...
app.post('/api/artifact/save', async (req) => {
    ...
    try {
        ...
        const filePath = path_1.default.join(artifactDir, `${name}.${type}`);
...
app.get('/api/artifact/:name', async (req, reply) => {
    ...
    const artifactDir = path_1.default.join(process.cwd(), '.artifacts');
    const filePath = path_1.default.join(artifactDir, name);
    try {
...
app.post('/api/execute', async (req) => {
    return (0, executor_1.executeCommand)(req.body.command);
});
// 2. LISP VM Backend
app.post('/api/lisp/eval', async (req) => {
    return lisp_vm_1.lispVM.eval(req.body.sexpr, req.body.agent_id);
});
// 3. MCP Router
app.post('/api/mcp/call', async (req) => {
    return (0, mcp_router_1.routeToMCP)(req.body);
});
// 4. Agent Pool
app.post('/api/agents/execute', async (req) => {
    return agent_pool_1.agentPool.execute(req.body.tasks);
});
// 5. WORM Chain Info
app.get('/api/worm/chain', async () => {
    return { ledger: worm_chain_1.wormChain.getChain().slice(-10) };
});
// WebSocket: Agent status streaming
app.get('/api/agents/:id/ws', { websocket: true }, (socket) => {
    if (!agent_pool_1.agentPool)
        return;
    agent_pool_1.agentPool.streamStatus(socket);
});
// =====================================================================
// START SERVER
// =====================================================================
